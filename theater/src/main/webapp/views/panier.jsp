...
<%@ include file="header.jsp"%>
	...
	<table class="table table-striped p-2">
					...
					<tr>
						...
						<td>
							<form action = "deleteArticle">
								<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
								<input type=hidden name=id value="${ligne.article.id}">
								<input type="submit" class="btn btn-sm" value="effacer">
							</form>
		...
		<tfoot>
			<tr>
				<td colspan="3" class="text-right">Prix total panier :</td>
				<td class="text-right"><c:out value="${panier.prixTotalPanier() }"></c:out></td>
			</tr>
		</tfoot>
	</table>
	...
	<div class="row">
		...
		<div class="text-center col-sm-2 col-md-2 col-lg-2">
			<form action="${contextPath }/addToCart" method="GET">
					<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
					<input class="btn btn-sm m-1 btn-primary" type="submit" value="Ajouter au panier">
			</form>
		</div>
		<div class="col-sm-4 col-md-4 col-lg-4"></div>
	</div>
